...
impute_hmean <-
  function(Year_Exp,
    ...
    ## Wind data taken from GSOD network: no humidity data in GHCND weather stations data.
    ...
    d = cbind(
      ...
      all_data[, colnames(all_data) %in% variable_to_impute],
      'dates' = as.character(as.vector(all_data$DATE))
    )
    d = as.data.frame(d)
    
    colnames(d)[3] = variable_to_impute
    ...
    if (nb_stations > 1) {
      ...
      saveRDS(to_save, paste('HMEAN_GSOD/', Year_Exp, '_IDW.RDS', sep = ''))
