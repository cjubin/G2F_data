...
`%notin%` <- Negate(`%in%`)
dat <- read.csv("g2f_2017_hybrid_data_clean.csv", header=T, stringsAsFactors=F)
dat$Year=2017
...
## Removed pheno. observations based on collaborators comments --> really a lot of different comments: easier to remove all obs. with a comment for that year
dat<-dat[-which(dat$Comments%notin%''),]
#critical_comments<-c('segregating for height','grain mixed at harvest',"Planter issue. Half of the plot was empty.","TW failed","left row behind","Only one packet of seed received; discovered during planting.  Other row filled in with misc. hybrid for competition",'Segregating for height. Error, Lost plot combining','Error, Lost plot combining','mixed', 'Not enough grain for accurate moisture reading','Racoon Damage' ,  'mammal damage'   ,'TW failed. 1 random main ear taken for ionomics' ,'Hit by field effect old road?' , 'Unable to collect grain moisture and test weight', 'Hand Harvested. Very immature, almost no grain fill','low yield, unreliable data' ,'discard','Discard', 'missing seed' ,'poor','harvest error',"Yield Company called it 'lost'",'combine damage')
...
write.table(dat,file = 'pheno2017_after_cleaning.txt',col.names = T,row.names = F,sep = '\t',quote = F)
...
for (v in unique(dat$location)) {
  ...
  if (length(unique(dat[dat$location == v &
    ...
    means1[[j]] = cbind('2017', v, as.data.frame(lsmeans(mod1, "pedigree"))[, c(1:2)])
...
write.table(df_final,file = 'pheno2017.txt',col.names = T,row.names = F,sep = '\t',quote = F)
