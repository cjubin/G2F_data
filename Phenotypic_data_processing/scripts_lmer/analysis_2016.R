...
`%notin%` <- Negate(`%in%`)
dat <- read.csv("g2f_2016_hybrid_data_clean.csv", header=T, stringsAsFactors=F)
dat$Year=2016
...
#dat[dat$Field.Location=='NYH2'&dat$Date.Harvested=='12/8/16','Field.Location']<-'NYH2.a'
#dat[dat$Field.Location=='NYH2'&dat$Date.Harvested=='11/15/16','Field.Location']<-'NYH2.b'

dat[dat$Field.Location=='ILH1'&dat$Date.Planted=="5/6/16",'Field.Location']<-'ILH1.a'
dat[dat$Field.Location=='ILH1'&dat$Date.Planted=="4/26/16",'Field.Location']<-'ILH1.b'
...
write.table(dat,file = 'pheno2016_after_cleaning.txt',col.names = T,row.names = F,sep = '\t',quote = F)
...
for (v in unique(dat$location)) {
  ...
  if (length(unique(dat[dat$location == v &
    ...
    means1[[j]] = cbind('2016', v, as.data.frame(lsmeans(mod1, "pedigree"))[, c(1:2)])
...
write.table(df_final,file = 'pheno2016.txt',col.names = T,row.names = F,sep = '\t',quote = F)
