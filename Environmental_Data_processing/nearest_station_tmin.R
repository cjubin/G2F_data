...
#'
#' \code{nearest_station_tmin} interpolates values for a specific meteorological variable from ISD stations given a time frame for a specific location
  ...
  function(Year_Exp,
    ...
    if (file.exists(
      ...
      nearest_station_values <-
        nearest_station_values[yday(nearest_station_values$date) %in% yday(dates), 'tmin']
      ...
      colnames(d)[which(colnames(d) == meteo_variable_GHCND)] = 'TMIN'
      ...
      colnames(predictions.table) = c('Year_Exp', 'TMIN', 'date', 'network','field_values')
      ...
      saveRDS(to_save, paste('TMIN_nearest/', Year_Exp, '_1station.RDS', sep = ''))
