...
impute_rhmean2 <-
  function(Year_Exp,
    ...
    if (Year_Exp %notin% c(
      ...
      if (nrow(all_data[all_data$STATION == min_dist_station, ]) > length(field_values)) {
        nearest_station_values <-
          all_data[all_data$STATION == min_dist_station, ]
        nearest_station_values <-
          nearest_station_values[match(yday(dates),yday(nearest_station_values$DATE)), c('HMEAN')]
    ...
    if (nb_stations == 1) {
      ...
      if (any(predictions.table$HMEAN>100)|any(predictions.table$HMEAN<0)){print('bug')}
    ...
    if (nb_stations > 1) {
      ...
      if(Year_Exp%in%c(
        ...
        if (any(pred$rhmin>100)|any(pred$rhmin<0)){print('bug')}
        if (any(pred$rhmax>100)|any(pred$rhmax<0)){print('bug')}
        ...
        if (length(field_values) > 1 & !all(is.na(field_values))) {
          cors_rhmin_field = cor(pred$rhmin, daily_weather[daily_weather$Year_Exp==Year_Exp,'HMIN'], use = 'complete.obs')
          cors_rhmax_field = cor(pred$rhmax, daily_weather[daily_weather$Year_Exp==Year_Exp,'HMAX'], use = 'complete.obs')
        ...
        to_save <-
          list(pred, cors_rhmin_field,cors_rhmax_field, min_dist_station, min_dist_name, min_dist)
        names(to_save) <-
          c(
            'predictions_YearExp',
            'cors_rhmin_field',
