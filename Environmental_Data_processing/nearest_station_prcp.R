...
#'
#' \code{nearest_station_prcp} interpolates values for a specific meteorological variable from ISD stations given a time frame for a specific location
...
nearest_station_prcp <-
  function(Year_Exp,
    ...
    if (file.exists(
      paste(
        'C:/Users/cathyjubin/Desktop/PRCP_allstations_elev/PRCP_',
        Year_Exp,
        '_elev',
        '.txt',
        sep = ''
      ...
      nearest_station_values <-
          nearest_station_values[match(yday(dates),yday(nearest_station_values$date)) , 'prcp']
      ...
      d = d[, c('prcp', 'date', 'longitude', 'latitude')]
      
      colnames(d)[which(colnames(d) == meteo_variable_GHCND)] = 'PRCP'
      ...
      colnames(predictions.table) = c('Year_Exp', 'PRCP', 'date', 'network','field_values')
      ...
      saveRDS(to_save, paste('PRCP_1station/all_stations/', Year_Exp, '_1station.RDS', sep = ''))
      ...
      if (nrow(d[d$station_id == min_dist_station,]) > length(dates)) {
        ...
        nearest_station_values <-
          nearest_station_values[yday(nearest_station_values$date) %in% yday(dates), 'prcp']
      }
      while (length(nearest_station_values) < length(dates)|length(which(is.na(nearest_station_values))) > 8) {
