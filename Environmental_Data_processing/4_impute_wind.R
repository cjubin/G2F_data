...
#'
#' \code{impute_wind2} interpolates values for a specific meteorological variable using Inverse Distance Weighting or nearby station if only 1 station.
...
impute_wind2 <-
  function(Year_Exp,
    ...
    if (Year_Exp %notin% c(
      ...
      ##Control for flagged values >90
      all_data=all_data[which(all_data$WDSP<90),]
      ...
      if (nrow(all_data[all_data$STATION == min_dist_station, ]) > length(field_values)) {
        nearest_station_values <-
          all_data[all_data$STATION == min_dist_station, ]
        nearest_station_values <-
          nearest_station_values[match(yday(dates),yday(nearest_station_values$DATE)), c('WDSP')]
      ...
      if (nrow(d[d$station_id == min_dist_station,]) > length(dates)) {
        ...
        nearest_station_values <-
          nearest_station_values[yday(nearest_station_values$date) %in% yday(dates), 'awnd']
      }
    ...
    if (nb_stations == 1) {
      ...
      to_save = list(predictions.table, cors, min_dist_station, min_dist,min_dist_name)
      names(to_save) <-
        c(
          'predictions_YearExp',
      ...
      saveRDS(to_save, paste('AWND_GSOD_WeatherCAN/', Year_Exp, '_1station.RDS', sep = ''))
    }
    ...
    if (nb_stations > 1) {
      ...
      pred$nearest_station_values <- 0.748 * nearest_station_values
      ...
      saveRDS(to_save, paste('AWND_GSOD_WeatherCAN/', Year_Exp, '_IDW.RDS', sep = ''))
