...
impute_kriging_withGHCND <- function(Year_Exp,radius=70,meteo_variable_GHCND,daily_weather=daily_weather,meteo_variable_in_table=NULL) {
  library(rnoaa)
  library(raster)
  library(mapdata)
  library(maps)
  ...
  all_data=lapply(stations_close$id, function(x)download_data(x,datatypeid = meteo_variable_GHCND))
